...
export const validator = {
  ...
  password: password => password && (new RegExp('^(?=.*[a-zA-Z])(?=.*[0-9])(?=.{6,})')).test(password),
  firstName: firstName => firstName && (new RegExp(/^[a-z ,.'-]+$/i)).test(firstName),
  ...
  productPrice: price => typeof price === 'object' &&
