...
const ProductDetailsCard = ({
  categories, getFieldDecorator, fileListError, fileList, onUploadImage, token,
}) => (
  <Card title="Product Details">
    ...
    <FormItem
      ...
      required
    >
      {getFieldDecorator('price.min', { initialValue: 150 })(<InputNumber
        min={1}
        formatter={value => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
        parser={value => value.replace(/\$\s?|(,*)/g, '')}
      />)} <div className="priceSeparate">~</div>
      {getFieldDecorator('price.max', { initialValue: 300 })(<InputNumber
        max={100000}
        formatter={value => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
        parser={value => value.replace(/\$\s?|(,*)/g, '')}
      />)}
    ...
    >
      ...
      >
        ...
        {fileListError && <div className="error">Please upload at least one image</div>}
      </Upload>)}
    </FormItem>
  </Card>
);
